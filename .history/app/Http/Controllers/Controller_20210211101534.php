        ...
        if ($request->file()) {
            ...
            foreach ($subscribers as $subscriber) {
                ...
                while ($startDay->diffInDays($subscriber->stop, false) >0) {
                    ...
                    if ($startDay->diffInDays($subscriber->stop, false) / 365 > 0) {
                        ...
                        Log::debug('Interval 365 ' . $subscription->start. '/'.$subscription->stop);

                        $subscriber->subscriptions[] = $subscription;
                        $startDay                    = $stopDay->addDays(1);

                    } else
                    if ($startDay->diffInDays($subscriber->stop, false) / 30 > 0) {
                        $stopDay = $startDay->copy();
                        $stopDay->addDays(30);
                        $subscription                = new Subscription();
                        ...
                        $subscription->start         = $startDay->format('Ymd');
                        $subscription->stop          = $stopDay->format('Ymd');
                        Log::debug('Interval 030 ' . $subscription->start. '/'.$subscription->stop);
                        $subscriber->subscriptions[] = $subscription;
                        $startDay                    = $stopDay->addDays(1);

                    } else
                    if ($startDay->diffInDays($subscriber->stop, false)/ 7 > 0) {
                        $stopDay = $startDay->copy();
                        $stopDay->addDays(7);
                        $subscription                = new Subscription();
                        ...
                        $subscription->start         = $startDay->format('Ymd');
                        $subscription->stop          = $stopDay->format('Ymd');
                        Log::debug('Interval 007 ' . $subscription->start. '/'.$subscription->stop);
                        $subscriber->subscriptions[] = $subscription;
                        $startDay                    = $stopDay->addDays(1);
                    ...
                    if ($startDay->diffInDays($subscriber->stop, false)/ 1 > 0) {
                        ...
                        $subscription->start         = $startDay->format('Ymd');
                        $subscription->stop          = $stopDay->format('Ymd');
                        Log::debug('Interval 001 ' . $subscription->start. '/'.$subscription->stop);
                        $subscriber->subscriptions[] = $subscription;
                        $startDay                    = $stopDay->addDays(1);
